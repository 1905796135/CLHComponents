...
@interface CycleCustomPageView () <UIScrollViewDelegate, UICollectionViewDataSource, UICollectionViewDelegate>
...
@implementation CycleCustomPageView
...
- (UICollectionViewScrollDirection)collectionViewScrollDirection {
    return self.scrollDirection == CycleCustomPageViewScrollDirectionHorizontal ? UICollectionViewScrollDirectionHorizontal : UICollectionViewScrollDirectionVertical;
}

- (UICollectionViewScrollPosition)collectionViewScrollPosition {
    return self.scrollDirection == CycleCustomPageViewScrollDirectionHorizontal ? UICollectionViewScrollPositionLeft : UICollectionViewScrollPositionTop;
}

- (void)_BasePageView_setup
{
    self.interval = 5.f;
    self.scrollDirection = CycleCustomPageViewScrollDirectionHorizontal;
    {
        ...
        [self.contentView registerClass:[UICollectionViewCell class] forCellWithReuseIdentifier:@"UICollectionViewCell"];
...
- (void)setScrollDirection:(CycleCustomPageViewScrollDirection)scrollDirection {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if (self.pageCount == 0 || self.totalPageCount == 0) {
        return;
    }
    ...
    BOOL horiztonScroll = (self.scrollDirection == CycleCustomPageViewScrollDirectionHorizontal);
    ...
    if ((NSInteger)scrollDirectionLength <= 0 ||
        (NSInteger)scrollDirectionOffset % (NSInteger)scrollDirectionLength != 0) {
        return;
    }
...
- (void)runAutoPage {
    ...
    if (self.totalPageCount && self.autoRunPage) {
        self.timer = [NSTimer scheduledTimerWithTimeInterval:self.interval target:self selector:@selector(runCycleCustomPageView) userInfo:nil repeats:YES];
