...
- (instancetype)initWithObjectCProperty:(objc_property_t)property {
    if (self = [super init]) {
        ...
        if ([attrSet containsObject:@"C"]) {
            _storedType = PBObjectStoredTypeCopy;
        } else if ([attrSet containsObject:@"&"]) {
            _storedType = PBObjectStoredTypeAssign;
        } else if ([attrSet containsObject:@"W"]) {
            _storedType = PBObjectStoredTypeWeak;
        } else {
            _storedType = PBObjectStoredTypeRetain;
        }

        _isReadOnly = [attrSet containsObject:@"R"];

        if (strcmp(rawPropertyType, @encode(BOOL)) == 0) {
            _rootType = PBObjectTypeRawNumber;
            _objTypeName = @"bool";
            ...
            _rootType = PBObjectTypeId;
        } else if ([typeAttribute hasPrefix:@"T@"]) {
            _rootType = PBObjectTypeClass;
...
- (void)setAssociatedObject:(id)object forKey:(NSString *)key policy:(PBAssociationPolicy)policy {
    [self.associatedObjectNames addObject:key.copy];
    if (policy == PBAssociationPolicyWeak) {
        if (object) {
            NSPointerArray *weakArray = [NSPointerArray weakObjectsPointerArray];
            [weakArray addPointer:(__bridge void *) (object)];
            objc_setAssociatedObject(self, (__bridge void *) key, weakArray, (objc_AssociationPolicy) PBAssociationPolicyRetainNonatomic);
        } else {
            objc_setAssociatedObject(self, (__bridge void *) key, NULL, (objc_AssociationPolicy) PBAssociationPolicyRetainNonatomic);
        }
    } else {
...
- (void)removeAssociatedObjectForKey:(NSString *)key {
    [self setAssociatedObject:nil forKey:key policy:PBAssociationPolicyAssign];
...
- (NSDictionary *)propertyInfos {
    ...
    while (targetClass != [NSObject class]) {
        ...
        for (i = 0; i < outCount; i++) {
            objc_property_t property = properties[i];
            PBObjectTypeWrapper *wrapper = [[PBObjectTypeWrapper alloc] initWithObjectCProperty:property];
            if (wrapper) {
