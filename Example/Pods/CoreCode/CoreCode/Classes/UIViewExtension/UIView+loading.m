...
NSString *const PBHudViewKey = @"PBHudViewKey";
...
- (void)showLoadingWithMessage:(NSString *)message hideAfter:(NSTimeInterval)second isModal:(BOOL)isModal {
    ...
    if (second > 0) {
        ...
        [self setAssociatedObject:hud forKey:PBHudViewKey policy:PBAssociationPolicyRetainNonatomic];
    }
    hud.userInteractionEnabled = isModal;
}

- (void)hideLoading {
    MBProgressHUD *hud = [self getAssociatedObjectForKey:PBHudViewKey];
    if (hud) {
        [hud hideAnimated:NO];
        [self removeAssociatedObjectForKey:PBHudViewKey];
    }
}
