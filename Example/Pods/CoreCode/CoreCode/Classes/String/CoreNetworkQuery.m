...
- (NSComparisonResult)caseInsensitiveCompare:(PBQueryPair *)aQueryPair {
    return [self.key caseInsensitiveCompare:aQueryPair.key];
}

@end

@implementation CoreNetworkQuery
...
+ (NSString *)queryStringFromParameters:(NSDictionary *)aParameters
    ...
    for (PBQueryPair *queryPair in queryPairs) {
    ...
    for (id nestedKey in [dictionary.allKeys sortedArrayUsingDescriptors:@[sortDescriptor]]) {
        ...
        if (nestedValue) {
            if ([nestedValue isKindOfClass:[NSDictionary class]]) {
                NSString *jsonStr = [CoreJSONUtil stringFromJSONObject:nestedValue options:0];
                [mQueryPairs addObject:[[PBQueryPair alloc] initWithKey:nestedKey value:jsonStr]];
                ...
                NSString *jsonStr = [CoreJSONUtil stringFromJSONObject:nestedArray options:0];
                [mQueryPairs addObject:[[PBQueryPair alloc] initWithKey:nestedKey value:jsonStr]];
            } else {
                [mQueryPairs addObject:[[PBQueryPair alloc] initWithKey:nestedKey value:nestedValue]];
            }
        }
    }
